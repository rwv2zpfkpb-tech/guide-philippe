...
public/
  map-style.json      – Custom Google Maps Stil (Light Mode)
  map-style-dark.json – Dunkles Pendant, s. „Karten-Dark-Mode" unten
```
...
- **CSV-Import** (`ImportModal`, Button „CSV-Import" neben „Add new"): zweistufig — Schritt „pick" (Datei-Auswahl, liest die Datei client-seitig per `file.text()` und schickt den Rohtext an `previewCsvImport`), Schritt „preview" (oben ein Spoon-Rating-Chip-Picker „Spoon-Rating für neue Einträge" — gilt für **alle** neuen Zeilen dieses Imports, praktisch wenn die CSV-Liste bereits einer einzigen Bewertungsstufe entspricht, Default „Remembering" wie zuvor hartkodiert; darunter Liste „Neu" mit Checkboxen, standardmäßig alle an, plus Liste „Bereits vorhanden" nur informativ/nicht auswählbar mit Verweis auf den passenden bestehenden Namen; Zeilen ohne extrahierte Place-ID zeigen einen „Maps ↗"-Link zur manuellen Zuordnung später). „Importieren" ruft `confirmCsvImport` mit der ausgewählten Teilmenge (inkl. `note`) + dem gewählten Spoon-Rating, neue Restaurants (immer `status: 'draft'`, mit per `resolvePlaceForImport` bestmöglich vorbefüllten Kartendaten und Notiz-als-Fazit, s. `csvImport.ts`) werden vorne in die Tabelle eingefügt. `confirmCsvImport` patcht den zurückgegebenen `spoon_rating`-Wert in-memory auf das gewählte Rating (dieselbe Korrektur wie bei `createRestaurant`) — sonst zeigt die Tabelle direkt nach dem Import kurzzeitig das falsche (Default-)Emoji, weil der zurückgegebene Datensatz vor dem `sync_restaurant_spoon_rating`-Trigger erfasst wurde
...
- Ist `NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID_DARK` nicht gesetzt, fällt Dark Mode auf die helle Map-ID zurück (kein Fehler). **Manueller Schritt in der Google Cloud Console nötig**, um Dark Mode tatsächlich zu sehen: eine zweite Map-ID anlegen und dort den Inhalt von `public/map-style-dark.json` als Kartenstil verknüpfen (gleiches Vorgehen wie beim bereits bestehenden hellen Stil), dann `NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID_DARK` setzen.
